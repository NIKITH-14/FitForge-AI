...
export default function RegisterPage() {
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-primary)', padding: '20px' }}>
            <div style={{ position: 'absolute', top: '10%', right: '25%', width: 500, height: 500, background: 'radial-gradient(circle, rgba(6,182,212,0.1) 0%, transparent 70%)', pointerEvents: 'none' }} />
            ...
            <div className="glass-card animate-float" style={{ width: '100%', maxWidth: 440, padding: '48px 40px' }}>
                ...
                <h1 style={{ fontSize: 28, fontWeight: 800, marginBottom: 6 }}>Start your journey</h1>
                <p style={{ color: 'var(--text-muted)', marginBottom: 32, fontSize: 14 }}>Create your account and let AI coach your training</p>

                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                    {[
                        { label: 'Full Name', icon: <User className="w-4 h-4" />, value: name, set: setName, type: 'text', placeholder: 'John Smith' },
                        { label: 'Email', icon: <Mail className="w-4 h-4" />, value: email, set: setEmail, type: 'email', placeholder: 'you@example.com' },
                    ].map(({ label, icon, value, set, type, placeholder }) => (
                        <div key={label}>
                            <label style={{ fontSize: 13, color: 'var(--text-muted)', display: 'block', marginBottom: 8, fontWeight: 500 }}>{label}</label>
                            <div style={{ position: 'relative' }}>
                                <div style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }}>{icon}</div>
                                <input type={type} className="input-field" style={{ paddingLeft: 40 }} placeholder={placeholder} value={value} onChange={(e) => set(e.target.value)} required />
                            </div>
                        </div>
                    ))}
                    ...
                    <div>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input type={showPw ? 'text' : 'password'} className="input-field" style={{ paddingLeft: 40, paddingRight: 44 }} placeholder="Min 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} required />
